# После ухода сети Макдональдс из России Тимур решил открыть свою небольшую бургерную. Цена каждого бургера в его ресторане определяется количеством ингредиентов, которые выбирает сам посетитель.
...
dat =ChainMap(bread, meat, sauce, vegetables, toppings)
for k, v in sorted(n.items(), key=lambda x: x[0]):
    counter += (dat[k] * v)
    print(f'{k.ljust(len(max(n, key=len)))} x {v}')
if len(f'{k.ljust(len(max(n, key=len)))} x {v}') >= 10:
    print('-' * (len(f'{k.ljust(len(max(n, key=len)))} x {v}')))
else:
    print("-" * len(f'ИТОГ: {str(counter)}р'))
print(f'ИТОГ: {counter}р')
